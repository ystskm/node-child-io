...
const Default = {
  pwd: process.env.PWD,
  timeout: 5000, // default execution limit = 5 sec
  replace: { },
  escape: TRUE,
  args: [ ],
...
module.exports = Object.assign(IO, { IO }); // support OLD style IO = require("child-io").IO;
// <-- END_OF_MAIN <--
...
function IO(args, options) {
  ...
  if(is('string', io_opts.wkdir) && io_opts.modules) {
    ...
    try {
      ...
      const dependencies = io_opts.pkgDeps = { };
      ...
      const pkgPath = io_opts.pkgPath = `${wd}/package.json`;
      // (2024.01.28 sakamoto) 既存のパッケージモジュールを上書きしないようにする。
...
function exec(action, options) {
  ...
  return Promise.resolve().then(()=>{
    ...
    if(io_opts.pkgPath) {
      ...
      if(Object.keys(io_opts.pkgDeps).length) {
        const action = `exec:npm i --save ${Object.keys(io_opts.pkgDeps).map(n=>io_opts.pkgDeps[n] == "*" ? n: `${n}@${io_opts.pkgDeps[n]}`).join(' ')}`;
        // console.log('NPM INSTALL ACTION: ' + action);
        fp.send({ action, type: CommunicateType.intra });
      }
    }
